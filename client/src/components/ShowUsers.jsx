...
function ShowUsers() {
  ...
  const handleSubCategory = () => {
    axios
      .get(
        "http://localhost:5001/api/categories/getallsubcategories/" + UserId,
        {
          headers: {
            Authorization: `Bearer ${import.meta.env.VITE_API_KEY}`,
            "Content-Type": "application/json",
  ...
  const UserDetails = user.filter((obj) => obj.email !== UserId);
  ...
  const handleSubCat = () => {
    axios
      .get(
        "http://localhost:5001/api/categories/getspecificcategory/" + subCat,
        {
          headers: {
            Authorization: `Bearer ${import.meta.env.VITE_API_KEY}`,
            "Content-Type": "application/json",
      ...
      .then((res) => {
        console.log(
          `Category for selected sub category ${subCat} is ${res.data} `
        );
        setCat(res.data);
      });
  };
